...
document.addEventListener("DOMContentLoaded", function () {
  let mensCategory = document.getElementsByClassName("shop-category-product");
  let urlParams = new URLSearchParams(window.location.search);
  let productCategory = urlParams.get("category");

  const menProduct = all_product.filter(
    (product) => product.category == productCategory
  );

  menProduct.forEach((product) => {
    let menscard = document.createElement("div");
    menscard.className = "card";
    menscard.innerHTML = `
    ...
    for (let i = 0; i < mensCategory.length; i++) {
      mensCategory[i].appendChild(menscard);
    }
